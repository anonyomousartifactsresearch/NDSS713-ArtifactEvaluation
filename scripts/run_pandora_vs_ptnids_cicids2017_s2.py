...
def evaluate_zero_shot_performance(model, unseen_df, support_df, feature_groups, label_map, device, label_column, unseen_class_label, save_dir="."):
    """
    ...
    if len(unseen_df) > 100: # Need at least 100 for adaptation pool
        ...
        for unseen_class in unseen_list:
            ...
            if not zsl_test_subset.empty:
                evaluate_zero_shot_performance(model, zsl_test_subset, zsl_support_df, feature_groups, train_dataset.label_map, device, label_column, unseen_class_label=unseen_class, save_dir=args.save_dir)
            else:
