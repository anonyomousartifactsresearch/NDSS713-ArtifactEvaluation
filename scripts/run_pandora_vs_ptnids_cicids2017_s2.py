# run_pandora_s2_cicids2017.py
# "As-is" conversion of the user's new code (hiding multiple classes)
# into the reproducible script structure from S1.
# This script is for "Scenario 2" (hiding a group of classes).
# UPDATED: Removed t-SNE visualization as requested.
...
# from sklearn.manifold import TSNE # <-- Removed as requested
import warnings
import random
import time
...
def evaluate_zero_shot_performance(model, unseen_df, support_df, feature_groups, label_map, device, label_column, unseen_class_label):
    """
    ...
    for i in range(n_way):
        ...
        if torch.any(indices):
            ...
            print(f"Warning: No ZSL support samples for prototype index {i}.")
    ...
    # Evaluate
    for batch_temp, batch_vol, _ in tqdm(unseen_loader, desc=f"Evaluating Zero-Shot Model on {unseen_class_label}"):
        batch_temp, batch_vol = batch_temp.to(device), batch_vol.to(device)
...
def main():
    parser = argparse.ArgumentParser(description="Reproducible PANDORA S2 (multi-class unseen) experiment.")
    parser.add_argument("--dataset_path", type=str, default="/home/avinash-awasthi/Downloads/Conferences/NDSS_2026/CICIDS2017/CICIDS2017_Ready.csv", help="Path to cicflowmeter CSV.")
    parser.add_argument("--label_column", type=str, default="Label", help="Label column name.")
    parser.add_argument("--unseen_classes", type=str, default="Web Attack,DoS", help="Comma-separated unseen classes.")
    ...
    model_save_path = os.path.join(args.save_dir, "best_model_s2_attention_triplet.pth")
    history_save_path = os.path.join(args.save_dir, "training_history_s2_attention_triplet.csv")
    plot_save_path = os.path.join(args.save_dir, "training_plots_s2_attention_triplet.png")
    ...
    if model:
        ...
        temp_feat_path = os.path.join(args.save_dir, "feature_importance_temporal_s2.png")
        ...
        vol_feat_path = os.path.join(args.save_dir, "feature_importance_volumetric_s2.png")
    ...
    # tsne_save_path = os.path.join(args.save_dir, "tsne_plot_s2_cicids2017.png")
    ...
    if len(unseen_df) > 100: # Need at least 100 for adaptation pool
        # Reserve 100 samples for adaptation experiments, use the rest for testing
        try:
            unseen_test_df, unseen_adapt_df_pool = train_test_split(unseen_df, test_size=100, stratify=unseen_df[label_column], random_state=42)
        except ValueError:
             print("Warning: Could not stratify unseen adaptation pool split. Splitting without stratification.")
             unseen_test_df, unseen_adapt_df_pool = train_test_split(unseen_df, test_size=100, random_state=42)
        ...
        for unseen_class in unseen_list:
            ...
            if not zsl_test_subset.empty:
                evaluate_zero_shot_performance(model, zsl_test_subset, zsl_support_df, feature_groups, train_dataset.label_map, device, label_column, unseen_class_label=unseen_class)
            else:
        ...
        shots_to_evaluate = [1, 5, 10, 20, 50] # User's new list
        for num_shots in shots_to_evaluate:
            ...
            # Subsample the unseen test data to use exactly 10000 samples per unseen class for evaluation.
            unseen_eval_df = unseen_test_df.groupby(label_column).head(10000)
        ...
        print(f"\nSkipping Zero-Shot & Adaptation Evaluation: Not enough samples in the unseen class (found {len(unseen_df)}, need > 100).")
