# run_pandora_ciciot2023_s_random.py
# "As-is" conversion of the user's new code for the CICIoT2023 dataset.
...
def split_features(all_features):
    ...
    if not temporal_features or not volumetric_features:
        ...
        # Create a copy to avoid shuffling the original list if it's used elsewhere
        temp_all_features = list(all_features)
...
# Function to Visualize Embeddings (REMOVED)
...
def plot_auc_roc_curve(model, test_loader, support_df, feature_groups, label_map, device, label_column, title="", save_path="roc_auc_plot.png"):
    ...
    if y_true_binarized.shape[1] != y_scores.shape[1]:
        print(f"Warning: Mismatch in binarized labels ({y_true_binarized.shape[1]}) and scores ({y_scores.shape[1]}). Adjusting arrays.")
        n_classes_to_plot = min(y_true_binarized.shape[1], y_scores.shape[1])
        y_true_binarized = y_true_binarized[:, :n_classes_to_plot]
        y_scores = y_scores[:, :n_classes_to_plot]
    else:
...
def evaluate_zero_shot_performance(model, unseen_df, support_df, feature_groups, label_map, device, label_column, is_adapted=False, unseen_class_label=""):
    ...
    if is_adapted:
        ...
        if len(results_df['Detected as Novelty']) > 0:
            ...
            print("\nNovelty Detection Accuracy: N/A (No samples detected)")
...
def main():
    parser = argparse.ArgumentParser(description="Reproducible SOTA IDS experiment on CICIoT2023.")
    parser.add_argument("--dataset_path", type=str, default="/home/beamformer/NDSS_2025/CICIoT2023_Ready.csv", help="Path to CICIoT2023 CSV.")
    ...
    model_save_path = os.path.join(args.save_dir, f"best_model_ciciot2023_hiding_{UNSEEN_CLASS_LABEL}.pth")
    history_save_path = os.path.join(args.save_dir, f"training_history_ciciot2023_hiding_{UNSEEN_CLASS_LABEL}.csv")
    plot_save_path = os.path.join(args.save_dir, f"training_plots_ciciot2023_hiding_{UNSEEN_CLASS_LABEL}.png")
    ...
    # --- Part 2: t-SNE Embedding Visualization (REMOVED) ---

    # --- Part 3: AUC ROC Curve Visualization ---
    auc_roc_save_path = os.path.join(args.save_dir, f"auc_roc_plot_ciciot2023_hiding_{UNSEEN_CLASS_LABEL}.png")
    plot_auc_roc_curve(model, test_loader, support_set_for_eval, feature_groups, train_dataset.label_map, device, label_column, "CICIoT2023 Seen Attacks", auc_roc_save_path)

    # --- Part 4: Zero-Shot & Adaptation Evaluation ---
    if len(unseen_df) > 10:
        ...
        evaluate_zero_shot_performance(model, unseen_zsl_test_df, zsl_support_df, feature_groups, train_dataset.label_map, device, label_column, is_adapted=False, unseen_class_label=UNSEEN_CLASS_LABEL)
