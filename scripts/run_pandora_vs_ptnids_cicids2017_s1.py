...
def evaluate_known_attacks(model, test_loader, support_df, feature_groups, label_map, device, title=""):
...
def evaluate_zero_shot_performance(model, unseen_df, support_df, feature_groups, label_map, device, label_col, unseen_class_label):
    """
...
def main():
    ...
    for unseen_class in unseen_list:
        ...
        evaluate_known_attacks(model, test_loader, support_set_for_eval, feature_groups, train_dataset.label_map, device=device, title="Seen Attacks")
        ...
        if len(unseen_df) > 10:
            ...
            evaluate_zero_shot_performance(model, unseen_zsl_test_df, zsl_support_df, feature_groups, train_dataset.label_map, device, label_column, unseen_class)
            ...
            for num_shots in shots_to_evaluate:
                ...
                evaluate_known_attacks(adapted_model, combined_loader, adapted_support_df, feature_groups, full_label_map, device, title=f"Adapted Model ({num_shots}-shot) on All Attacks")
        else:
