# run_cicids2018_zsl.py
# Direct conversion of CICIDS2018_Proposed_ZSL_5_shot.ipynb -> python
...
def evaluate_on_test_set(model, test_loader, train_df, feature_groups, label_map, device, label_column, title="", save_dir="."):
    if model is None: return
    model.eval()
    y_true, y_pred = [], []
    print(f"\n--- Evaluating model performance on the {title} ---")

    # Create prototypes from the entire training data
    train_df[label_column] = train_df[label_column].astype('category')
    train_df['label_code'] = train_df[label_column].cat.codes
    support_labels = torch.tensor(train_df['label_code'].values, dtype=torch.long)

    unique_labels = torch.unique(support_labels)
    n_way = len(unique_labels)

    s_temp = torch.tensor(train_df[feature_groups['temporal']].values, dtype=torch.float32).to(device)
    s_vol = torch.tensor(train_df[feature_groups['volumetric']].values, dtype=torch.float32).to(device)
    ...
    if title:
        ...
        csv_path = os.path.join(save_dir, f"report_{safe_title}.csv")
...
def main():
    parser = argparse.ArgumentParser(description="Reproducible conversion of CICIDS2018 notebook to .py")
    parser.add_argument("--dataset_path", type=str, default="/home/avinash-awasthi/Downloads/Conferences/NDSS_2026/CICIDS2018_Domain_Shift-Ready_Again.csv", help="Path to CICIDS2018 CSV.")
    ...
    print("\nTraining on all classes for CICIDS2018.")
    ...
    patience_counter = 0
    model_save_path = os.path.join(args.save_dir, 'best_model_cicids2018.pth')
    ...
    history_csv_path = os.path.join(args.save_dir, 'training_history_cicids2018.csv')
    history_df = pd.DataFrame(history)
    history_df.to_csv(history_csv_path, index=False)
    print(f"Training history saved successfully to {history_csv_path}")

    # --- Cell 9 ---
    plot_save_path = os.path.join(args.save_dir, 'training_plots_cicids2018.png')
    plot_training_history(history, plot_save_path)

    # --- Cell 10 ---
    plot_feature_importance(model, feature_groups, args.save_dir, "cicids2018")
    ...
    full_label_map = dict(enumerate(final_df[label_column].astype('category').cat.categories))

    evaluate_on_test_set(model, test_loader, train_df, feature_groups, full_label_map, device, label_column, title="CICIDS2018 Test Set", save_dir=args.save_dir)
