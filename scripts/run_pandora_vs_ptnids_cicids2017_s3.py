# run_pandora_s3_cicids2017.py
# "As-is" conversion of the user's new code (hiding DDoS, PortScan, Bot)
# into the reproducible script structure.
# This script is for "Scenario 3".
# UPDATED: Fixed TSNE argument 'n_iter' to 'max_iter'.
...
# from sklearn.manifold import TSNE # <-- Removed
import warnings
import random
import time
...
def apply_class_mapping(df, label_column):
    if df.empty:
        return df
    # Using the more robust S1 reference mapping
    CLASS_MAPPING = {
        'Benign': 'Benign',
        'Bot': 'Bot',
        'Ddos': 'DDoS',
...
class PANDORA_IDS_Probabilistic(nn.Module):
...
print("Full Probabilistic PANDORA IDS model defined with combined Triplet Loss and Feature Attention.")
...
# --- Removed visualize_embeddings function ---

def evaluate_known_attacks(model, test_loader, support_df, feature_groups, label_map, device, label_column, title=""):
...
def evaluate_zero_shot_performance(model, unseen_df, support_df, feature_groups, label_map, device, label_column, is_adapted=False, unseen_class_label=""):
    """
    ...
    for i in range(n_way):
        ...
        if torch.any(indices):
            ...
            print(f"Warning: No ZSL/Adapt support samples for prototype index {i}.")
    ...
    # Evaluate
    for batch_temp, batch_vol, _ in tqdm(unseen_loader, desc=f"Evaluating {'Adapted' if is_adapted else 'Zero-Shot'} Model on {unseen_class_label}"):
        batch_temp, batch_vol = batch_temp.to(device), batch_vol.to(device)
    ...
    if is_adapted:
        ...
        # Note: In a multi-class adaptation, this report might be complex.
        # The original code prints a classification report.
        report = classification_report(y_true, y_pred, zero_division=0, labels=sorted(list(set(y_true + y_pred))))
        print(report)
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Reproducible PANDORA S3 (multi-class unseen) experiment.")
    parser.add_argument("--dataset_path", type=str, default="/home/avinash-awasthi/Downloads/NDSS_2025/CICIDS2017/CICIDS2017_Ready.csv", help="Path to cicflowmeter CSV.")
    parser.add_argument("--label_column", type=str, default="Label", help="Label column name.")
    parser.add_argument("--unseen_classes", type=str, default="DDoS,PortScan,Bot", help="Comma-separated unseen classes.")
    ...
    if df.empty:
        print("Empty dataframe after mapping — exiting.")
        return
    ...
    # --- Data Splitting (S3 Logic) ---
    print(f"\n==================== Running experiment hiding unseen classes: {unseen_list} ====================")
    seen_df = final_df[~final_df[label_column].isin(unseen_list)]
    unseen_df = final_df[final_df[label_column].isin(unseen_list)]

    if seen_df.empty:
        print(f"Error: No data left after excluding unseen classes. Exiting.")
        return

    if unseen_df.empty:
        print(f"Warning: The unseen classes {unseen_list} are not in the dataset. ZSL/Adaptation will be skipped.")
    ...
    # Model init
    model = PANDORA_IDS_Probabilistic(
    ...
    # Define save paths based on save_dir
    model_save_path = os.path.join(args.save_dir, "best_model_s3_attention_triplet.pth")
    history_save_path = os.path.join(args.save_dir, "training_history_s3_attention_triplet.csv")
    plot_save_path = os.path.join(args.save_dir, "training_plots_s3_attention_triplet.png")
    ...
    if model:
        ...
        temp_feat_path = os.path.join(args.save_dir, "feature_importance_temporal_s3.png")
        ...
        vol_feat_path = os.path.join(args.save_dir, "feature_importance_volumetric_s3.png")
    ...
    evaluate_known_attacks(model, test_loader, support_set_for_eval, feature_groups, train_dataset.label_map, device, label_column, title="Seen Attacks")

    # --- Part 2: Embedding Visualization (REMOVED) ---
    # tsne_save_path = os.path.join(args.save_dir, "tsne_plot_s3_cicids2017.png")
    # visualize_embeddings(model, final_df, feature_groups, label_column, device, "CICIDS2017 Dataset", tsne_save_path)
    print("\nSkipping t-SNE visualization as requested.")

    # --- Part 3: Zero-Shot & Adaptation Evaluation ---
    if len(unseen_df) > 10: # Need at least 10 for adaptation pool
        ...
        try:
            unseen_test_df, unseen_adapt_df_pool = train_test_split(unseen_df, test_size=10, stratify=unseen_df[label_column], random_state=42)
             ...
             print("Warning: Could not stratify unseen adaptation pool split (need >= 2 samples per class). Splitting without stratification.")
        ...
        # STAGE 1: PURE ZERO-SHOT EVALUATION (Loop through each unseen class)
        zsl_support_df = train_df.groupby(label_column).head(10)
        for unseen_class in unseen_list:
            zsl_test_subset = unseen_zsl_test_df[unseen_zsl_test_df[label_column] == unseen_class]
            if not zsl_test_subset.empty:
                evaluate_zero_shot_performance(model, zsl_test_subset, zsl_support_df, feature_groups, train_dataset.label_map, device, label_column, is_adapted=False, unseen_class_label=unseen_class)
            else:
                print(f"No ZSL test samples for class {unseen_class}. Skipping ZSL eval for it.")


        # STAGE 2: FEW-SHOT ADAPTATION & RE-EVALUATION LOOP
        shots_to_evaluate = [1, 5, 10] # User's new list for this script
        for num_shots in shots_to_evaluate:
            ...
            # Sample the adaptation shots from the pool
            # Ensure we can stratify if possible, or just sample
            try:
                # Proportional stratified sampling
                unseen_adapt_df = unseen_adapt_df_pool.groupby(label_column, group_keys=False).apply(lambda x: x.sample(n=max(1, int(np.ceil(num_shots * len(x) / len(unseen_adapt_df_pool)))), random_state=42))
                # If this method got fewer samples than desired (due to rounding), top up
                if len(unseen_adapt_df) < num_shots:
                    additional_samples = unseen_adapt_df_pool.drop(unseen_adapt_df.index).sample(n=(num_shots - len(unseen_adapt_df)), replace=False, random_state=42)
                    unseen_adapt_df = pd.concat([unseen_adapt_df, additional_samples])
                # If we got more (due to `max(1, ...)`), trim it back down
                if len(unseen_adapt_df) > num_shots:
                    unseen_adapt_df = unseen_adapt_df.sample(n=num_shots, random_state=42)
            ...
            print(f"Adaptation using {num_shots} shots. Actual samples used: {len(unseen_adapt_df)}")
            print("Shot distribution:")
            print(unseen_adapt_df[label_column].value_counts())


            # Create the training set for fine-tuning (using 20000 seen samples)
            adaptation_training_df = pd.concat([train_df.sample(n=20000, random_state=42), unseen_adapt_df])
            ...
            # ✅ **MODIFICATION START (from user's code)**
            # Subsample the unseen test data to use exactly 20000 samples per unseen class for evaluation.
            unseen_eval_df = unseen_test_df.groupby(label_column).head(20000)

            # Create the combined test dataframe using the full seen test set and the subsampled unseen test set.
            combined_test_df = pd.concat([test_df, unseen_eval_df])
            
            print(f"INFO: Evaluation test set created with {len(test_df)} 'seen' samples and {len(unseen_eval_df)} 'unseen' samples.")
            # ✅ **MODIFICATION END**
            ...
            full_label_map_df = pd.concat([train_df, unseen_df])
            full_label_map_df[label_column] = full_label_map_df[label_column].astype('category')
            full_label_map = dict(enumerate(full_label_map_df[label_column].cat.categories))
            ...
            evaluate_known_attacks(adapted_model, combined_loader, adapted_support_df, feature_groups, full_label_map, device, label_column, title=f"Adapted Model ({num_shots}-shot) on All Attacks")

    else:
        print(f"\nSkipping Zero-Shot & Adaptation Evaluation: Not enough samples in the unseen class (found {len(unseen_df)}, need > 10).")
