...
def evaluate_all_attacks(model, test_loader, support_df, feature_groups, label_map, device, label_column, title="", save_dir="."):
...
def evaluate_adapted_model(model, query_loader, support_df, feature_groups, label_map, device, label_column, title="", save_dir="."):
    ...
    if title:
        ...
        # 1. Classification Report CSV
        report_dict = classification_report(y_true, y_pred, labels=all_present_labels, zero_division=0, output_dict=True)
        report_df = pd.DataFrame(report_dict).transpose()
        csv_path = os.path.join(save_dir, f"report_{safe_title}.csv")
        report_df.to_csv(csv_path)
        print(f"Saved classification report to {csv_path}")
...
def run_domain_shift_adaptation_experiment(base_model, cross_dataset_path, training_feature_groups, full_label_map, device, label_column, all_categories, save_dir="."):
    ...
    for n_shot in [1, 5]:
        ...
        evaluate_adapted_model(adapted_model, query_loader, support_df, training_feature_groups, full_label_map, device, label_column, title=f"CICIDS2018 Adapted on {n_shot}-Shot (Re-Scaled)", save_dir=save_dir)
...
def main():
    ...
    evaluate_all_attacks(model, test_loader, train_df, feature_groups, full_label_map, device, label_column, title="CICIDS2017 Test Set", save_dir=args.save_dir)
